...
def get_model():
    ...
    model = dereflection.LocationAwareSIRR()
...
def image_client(name, input_files, output_dir):
    ...
    for filename in image_filenames:
        ...
        context = cmd.dereflection(filename, output_file)
...
def image_server(name, host="localhost", port=6379):
    ...
    return redos.image.service(name, "image_dereflection", do_service, host, port)
...
def video_service(input_file, output_file, targ):
    ...
    print(f"  dereflection {input_file}, save to {output_file} ...")
    progress_bar = tqdm(total=video.n_frames)

    def dereflection_video_frame(no, data):
    ...
    video.forward(callback=dereflection_video_frame)
...
def video_client(name, input_file, output_file):
    ...
    context = cmd.dereflection(input_file, output_file)
    redo = redos.Redos(name)
    redo.set_queue_task(context)
...
def video_server(name, host="localhost", port=6379):
    return redos.video.service(name, "video_dereflection", video_service, host, port)
