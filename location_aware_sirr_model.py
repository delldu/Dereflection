...
class Conv2DLayer(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "conv2d",
            nn.Conv2d(
                in_channels, out_channels, k_size, stride, padding, dilation=dilation, bias=bias
            ),
        )
        if norm is not None:
...
class LaplacianPyramid(nn.Module):
    ...
    def forward(self, x):
        ...
        lap_0 = F.conv2d(
            x0, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1
        )
        lap_1 = F.conv2d(
            x1, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1
        )
        lap_2 = F.conv2d(
            x2, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1
        )
        lap_3 = F.conv2d(
            x, self.kernel, groups=self.channel_dim, padding=1, stride=1, dilation=1
        )
...
class LRM(nn.Module):
    ...
    def __init__(self):
        ...
        # SE-resblocks(P-ReLU)
        self.det_conv6 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
        self.det_conv7 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
        self.det_conv8 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
        self.det_conv9 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
        self.det_conv10 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
        self.det_conv11 = ResidualBlock(
            32, norm=None, se_reduction=2, res_scale=0.1, act=nn.PReLU()
        )
...
class LocationAwareSIRR(nn.Module):
    def __init__(self, opts, device):
        ...
        self.visual_names = ["fake_Ts", "I"]

        self.netG_T = LRM().to(device)
    ...
    def init(self):
        ...
        self.rcmaps = []  # reflection confidence map
    ...
    def forward(self):
        ...
        # pdb.set_trace()
        # self.I.size() -- torch.Size([1, 3, 400, 600])
        # len(self.fake_Ts) -- 4
        # len(self.fake_Rs) -- 4
